...
from app.pricing.tiles import INSTALLATION_METHOD_LABELS
...
def build_tiles_pdf(quotation: Quotation) -> bytes:
    ...
    installation_label = INSTALLATION_METHOD_LABELS[installation_method]
    ...
    elements.append(Paragraph(f"Tiles &amp; Flooring Supply &amp; Installation — {tier_name} Finish", doc_subtitle_style))
    ...
    info_rows = [
        ...
        ["Site Address:", payload.get("customer_address", ""), "Flooring Area:", f"{_format_qty(quotation.area_sqft)} sq.ft"],
        ["Prepared By:", PREPARED_BY, "Installation Method:", installation_label],
    ]
    info_table = Table(
        [[Paragraph(c, label_style if i % 2 == 0 else value_style) for i, c in enumerate(row)] for row in info_rows],
        colWidths=[26 * mm, 59 * mm, 34 * mm, 51 * mm],
    )
    info_table.setStyle(
        TableStyle(
            [
    ...
    elements.append(
        Paragraph(
            "This quotation covers supply and on-site installation of tile flooring as per the area, finish and "
            "installation method specified below. Rates are inclusive of material and labour for the item "
            "described; final quantities to be verified on site.",
            body_style,
        )
    )
    ...
    elements.append(Paragraph(f"<b>1.&nbsp;&nbsp;Tiles &amp; Flooring — {tier_name} Finish</b>", body_style))
    elements.append(Paragraph(", ".join(included_items) + ".", note_style))
    elements.append(Spacer(1, 2 * mm))

    line_desc = (
        f"Tiles &amp; Flooring — {tier_name} finish, {installation_label.lower()} "
        f"({_format_qty(quotation.area_sqft)} sq.ft @ {format_inr(payload['rate_per_sqft'])}/sq.ft)"
    )
    bom_data = [
        ...
        [
            Paragraph(f"<b>Section Total — Tiles &amp; Flooring — {tier_name} Finish</b>", table_cell_style),
    ...
    summary_data = [
        ...
        [
            ...
            Paragraph(f"Tiles &amp; Flooring — {tier_name} Finish", table_cell_style),
    ...
    terms = [
        "Prices are based on the tile grade and installation method specified above. Any upgrade in tile brand, size or design will be quoted and adjusted separately.",
        ...
        "Warranty: 90 days workmanship warranty from date of handover. Tiles carry the respective manufacturer's warranty.",
        "Scope excludes: sub-floor levelling/civil work beyond normal surface preparation, waterproofing, skirting upgrades, and loose furniture, unless explicitly listed above.",
