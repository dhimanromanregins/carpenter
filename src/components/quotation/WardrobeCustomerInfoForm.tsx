...
import { createWardrobeQuotation } from "@/api/wardrobe";
import { useQuotationStore } from "@/store/quotationStore";
import { ApiError } from "@/api/client";

export function WardrobeCustomerInfoForm({ onGenerated }: { onGenerated: (quotationId: number) => void }) {
  const area = useQuotationStore((s) => s.area);
  const wardrobeQuote = useQuotationStore((s) => s.wardrobeQuote);
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!canSubmit || !wardrobeQuote || !area.areaSqft) return;

    setSubmitting(true);
    setError(null);
    try {
      const saved = await createWardrobeQuotation({
        area_sqft: area.areaSqft,
        tier: wardrobeQuote.tier,
  ...
  return (
    <GlassCard className="mx-auto max-w-xl p-8 md:p-10">
      ...
      <p className="mt-2 text-sm text-grey">We'll use these to prepare your wardrobe quotation PDF.</p>
      ...
      <form onSubmit={handleSubmit} className="mt-6 space-y-5">
        ...
        <div className="flex gap-3 pt-2">
          <MagneticButton variant="ghost" type="button" onClick={() => setStage("wardrobeTier")}>
            Back
          </MagneticButton>
