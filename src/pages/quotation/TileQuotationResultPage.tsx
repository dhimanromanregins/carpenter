...
import { getTileQuotation, getTileQuotationPdfUrl } from "@/api/tiles";
import { ApiError } from "@/api/client";
import { useSeo } from "@/hooks/useSeo";
import type { TileSavedOut } from "@/types/quotation";

const INSTALLATION_LABELS = { cement: "Cement Fixing", chemical: "Chemical Fixing" };

export function TileQuotationResultPage() {
  const { quotationId } = useParams<{ quotationId: string }>();
  const [quotation, setQuotation] = useState<TileSavedOut | null>(null);
          ...
          <>
            ...
            <div className="mt-6 space-y-1 text-sm">
              ...
              <div className="flex justify-between text-grey">
                ...
                <span className="text-cream">{INSTALLATION_LABELS[quotation.installation_method]}</span>
              </div>
              <div className="flex justify-between text-grey">
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <a
                href={getTileQuotationPdfUrl(quotation.id)}
                target="_blank"
                rel="noreferrer"
