...
import { getCeilingQuotation, getCeilingQuotationPdfUrl } from "@/api/ceiling";
import { ApiError } from "@/api/client";
import { useSeo } from "@/hooks/useSeo";
import type { CeilingSavedOut } from "@/types/quotation";

export function CeilingQuotationResultPage() {
  const { quotationId } = useParams<{ quotationId: string }>();
  const [quotation, setQuotation] = useState<CeilingSavedOut | null>(null);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    getCeilingQuotation(quotationId)
      .then((data) => {
        ...
        {quotation && (
          <>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <a
                href={getCeilingQuotationPdfUrl(quotation.id)}
                target="_blank"
                rel="noreferrer"
